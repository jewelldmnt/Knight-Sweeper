...
def draw_board():
    ...
    for i in range(32):
        ...
        if is_putting_poison_done:
            ...
            screen.blit(font.render(status_text[turn_step], True, 'black'), (20, 620))
        else:
            status_text = ['Red: Choose where to put the poison apple', 'Green: Choose where to put the poison apple']
            screen.blit(font.render(status_text[turn_step_putting_poison], True, 'black'), (20, 620))
        ...
        screen.blit(font.render(golden_text, True, 'black'), (20, 660))
...
def draw_pieces():
    ...
    for i in range(len(red_pieces)):
        ...
        if red_pieces[i] == 'knight':
            screen.blit(red_images[index], (x * 75 + 10, y * 75 + 10))
        else:
            screen.blit(red_images[index], (x * 75 + 20, y * 75 + 18))
            value = red_apple_values.get((x, y), 0)
            screen.blit(font.render(str(value), True, 'white'), (x * 75 + 35, y * 75 + 28))
        if turn_step < 2 and selection == i and red_pieces[i] == 'knight':
            pygame.draw.rect(screen, 'yellow', [red_locations[i][0] * 75 + 1, red_locations[i][1] * 75 + 1, 
                             75, 75], 2)
    ...
    for i in range(len(green_pieces)):
        ...
        if green_pieces[i] == 'knight':
            screen.blit(green_images[index], (x * 75 + 10, y * 75 + 10))
        else:
            screen.blit(green_images[index], (x * 75 + 20, y * 75 + 18))
            value = green_apple_values.get((x, y), 0)
            screen.blit(font.render(str(value), True, 'white'), (x * 75 + 35, y * 75 + 28))
        if turn_step >= 2 and selection == i and green_pieces[i] == 'knight':
            pygame.draw.rect(screen, 'green', [green_locations[i][0] * 75 + 1, green_locations[i][1] * 75 + 1, 
                             75, 75], 5)
...
def draw_valid(moves):
    ...
    for i in range(len(moves)):
        pygame.draw.circle(screen, color, (moves[i][0] * 75 + 40, moves[i][1] * 75 + 40), 5)
...
def draw_captured_values():
    ...
    y_offset = 20
    for line in lines:
        screen.blit(font.render(line, True, 'black'), (620, y_offset))
        y_offset += 60
